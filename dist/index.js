import require$$0, { useState, useEffect, useMemo, createContext, useRef, useCallback, useContext } from 'react';
...
const SecretMenu = ({ secretMenu, toggleFlag, flags, secretMenuStyles, }) => {
    ...
    return (showMenu && (jsxRuntimeExports.jsxs("div", { style: styles.container, children: [jsxRuntimeExports.jsx("button", { style: styles.closeButton, onClick: () => {
                    setShowMenu(false);
                    setKeySequence([]);
                }, children: "X" }), jsxRuntimeExports.jsx("h1", { children: "Secret Menu" }), formattedFlags.map(({ key, name, enabled }) => (jsxRuntimeExports.jsxs("div", { style: styles.button, children: [jsxRuntimeExports.jsx("span", { children: name }), jsxRuntimeExports.jsx("button", { onClick: () => handleToggle(key), children: enabled ? "Enabled" : "Disabled" })] }, `sm_item_${key}`)))] })));
};
...
const SetFlagsContext = createContext(undefined);
const logIt = (...message) => {
...
const FlagsProvider = ({ options = {}, children, }) => {
    ...
    const fetchFlags = useCallback(async () => {
        ...
        try {
            ...
            const newFlags = data.flags ? data.flags.reduce((acc, flag) => ({
                ...acc,
    ...
    return (jsxRuntimeExports.jsx(SetFlagsContext.Provider, { value: setFlags, children: jsxRuntimeExports.jsxs(FlagsContext.Provider, { value: flags, children: [children, secretMenu?.length >= 1 && (jsxRuntimeExports.jsx(SecretMenu, { secretMenu: secretMenu, flags: flags, toggleFlag: toggleFlag, secretMenuStyles: secretMenuStyles }))] }) }));
};
const useFlags = () => {
    ...
    const toggle = useCallback((flagName) => {
    ...
    const initialize = useCallback((flag, defaultValue = false) => {
